...
void HHeaderItemDelegate::paint ( QPainter * painter, const QStyleOptionViewItem & option, const QModelIndex & index ) const
{
    ...
    //判断是否有checkbox
    if (HHeaderView::_Instance->isCheckedEnable(row, col))
    {
    ...
    //判断是否能够排序  
    if (HHeaderView::_Instance->isSortedEnable(row, col))
    {//可以排序
        //获取它当前的排序顺序
        int ret = HHeaderView::_Instance->sortedDirection(row, col);
        if (0 == ret)
        {
    ...
    if (HHeaderView::_Instance->isItemPress(row, col))
    {
   ...
   //             header_opt.state |= QStyle::State_Sunken;
    ...
    {//判断是该单元格是否有checkbox
        ...
        //判断checkbox的状态
        Qt::CheckState state = HHeaderView::_Instance->checkedState(row, col);
   ...
   // xopt.rect = option.rect;
    ...
    if (!HHeaderView::_Instance->isColHide(col))
    {
        ...
        if (header_opt.state & QStyle::State_MouseOver)
        {
            ...
            if (HHeaderView::_Instance->isCustomMenuEnable(row, col))
            {
        ...
        if (HHeaderView::_Instance->isSectionHidden(col - 1))
        {
...
void HHeaderItemDelegate::drawMenuBtn(int row, int col, const QRect& rect, QPainter* painter) const
{
    ...
    int state = HHeaderView::_Instance->customMenuState(row, col);
