...
DataTreeView* DataTreeView::_Instance = NULL;
DataTreeView::DataTreeView(QWidget* parent) : QTreeView(parent)
{
    _Instance = this;
...
void DataTreeView::onCollapsed ( const QModelIndex & index )
{
    ...
    for (int i = 0; i < size; ++i)
    {
        ...
        VHeaderView::_Instance->hideSection(serial);             
    }
...
void DataTreeView::onExpanded ( const QModelIndex & index )
{
    ...
    {
        int serial = NodeManager::getInstance()->serialNoOfNode(childNodeList.at(i));
        VHeaderView::_Instance->showSection(serial);      
    }
...
void DataTreeView::mouseReleaseEvent ( QMouseEvent * event )
{
    ...
    //调用竖头的函数，画三角形，表示该行选中
    VHeaderView::_Instance->selectRow(row);
    ...
    if (2 == col)
    {
        ...
        if (checkRect.contains(event->pos()))
        {
            ...
            HHeaderView::_Instance->setCheckedState(1, 2, Qt::PartiallyChecked);

            //立刻更新界面
            viewport()->update();
            HHeaderView::_Instance->viewport()->update();
        }
    }
    else if (0 == col)
    {      
        ...
        if (checkRect.contains(event->pos()))
        {
            ...
            HHeaderView::_Instance->setCheckedState(1, 1, Qt::PartiallyChecked);
