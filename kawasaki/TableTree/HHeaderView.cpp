...
void HHeaderView::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    if (isCustomMenuEnable(row, col))
    {
        ...
        if (rect.contains(pos))
        {
            _actHideCol->setEnabled(hiddenSectionCount() < count() - 1);
            QAction* act = _menu->exec(mapToGlobal(event->pos()));
            if (act == _actHideCol)
            {
                _hideColSet.insert(col); // 保存到隐藏列表中

                hideSection(col);
                //updateSection(col - 1);             
            }
            else if (act == _actsortUp)
            {
                sortUp(row, col);
            }
            else if (act == _actSortDown)
            {
                sortDown(row, col);
            }
        }
...
{
    if (!_pressIndex.isValid())
        return false;

    if (row == _pressIndex.row() && col == _pressIndex.column())
        return true;
