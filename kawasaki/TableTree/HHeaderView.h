...
private:
    ...
    QModelIndex _pressIndex; //保存按下的列

    //保存所有item的矩形 key:row  key1:col
    QMap<int, QMap<int, QRect> > _itemRectMap;

    //保存每个item能支持的style
    QMap<int, QMap<int, ItemState> > _itemStateMap;

    QSet<int> _hideColSet; //保存隐藏列的记录

private:
    QMenu* _menu;
    QAction* _actHideCol; //隐藏列
    QAction* _actsortUp;  //升序
    QAction* _actSortDown; //降序
};
